...
Confluent Hub
^^^^^^^^^^^^^
The plugin is hosted on the `Confluent Hub <https://www.confluent.io/hub/com.caleb.kafka.connect/QueryBasedJdbcConnector>`_. Installation through the
...
#. Create a subdirectory called `QueryBasedJdbcConnector` under the `plugin.path` on your connect worker.
